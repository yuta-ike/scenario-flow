...
export const Select = <Id extends string | null>({
  ...
  <RxSelect.Root
    ...
    onValueChange={(value) => onChange(value as Id)}
  >
    <RxSelect.Trigger className="flex w-full items-center gap-2 rounded border border-transparent px-1.5 py-0.5 text-sm hover:border-slate-200 data-[state=open]:border-slate-200">
      <RxSelect.Value />
      <RxSelect.Icon>
        <TbChevronDown />
      </RxSelect.Icon>
    ...
    <RxSelect.Portal>
      <RxSelect.Content
        position="popper"
        className="rounded border-slate-200 bg-white shadow-object"
      >
